...
pub enum PacketHeader {
    ...
    CloseConnection,
    /// Data packet
    Payload { response_length: u16, payload_length: u16}
}
...
impl<'a> PacketHeader {
    ...
    pub fn payload_length(&self) -> usize {
        match self {
            ...
            PacketHeader::Payload { payload_length,  .. }  => PACKET_HEADER_RECEIVE_SIZE + *payload_length as usize
        }
    }
    ...
    pub fn fill_header(&self, header: &mut [u8]) {
        ...
        match self {
            ...
            PacketHeader::Payload { response_length: receive_length, .. } => {
                let receive_bytes = receive_length.to_be_bytes();
...
mod tests {
    ...
    fn receive_packet(packet_bytes: &mut [u8], connection_id: u8, internal: bool) -> Result<(PacketHeader, Box<[u8]>), IrcError> {
        ...
        let packet = {
            ...
            if internal {
                ...
                irc.receive(&mut payload_ioslice)?
            }
        };

        if let PacketHeader::Payload{payload_length, ..} = packet {
    ...
    fn test_receive_packet(packet_bytes: &mut [u8], expected_packet: PacketHeader, expected_payload: &[u8], connection_id: u8, ignore_link_state: bool) {
        ...
        if let PacketHeader::Payload{payload_length, ..} = packet {
            assert_eq!(&payload[..payload_length as usize], expected_payload);
        }
    }
    ...
    fn send_small_payload_packet() {
        ...
        let packet = PacketHeader::Payload { response_length: 0x0010, payload_length: payload[0].len() as u16 };
    ...
    fn receive_payload_packet_small() {
        ...
        let header = PacketHeader::Payload { response_length: 0x0000, payload_length: 0x0001 };
    ...
    fn receive_payload_packet_large() {
        ...
        let header = PacketHeader::Payload { response_length: 0x5F4, payload_length: payload_bytes.len() as u16 };
